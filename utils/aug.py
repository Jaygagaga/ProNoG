...
def aug_random_edge_s(input_adj, drop_edge=0.3):
    percent =drop_edge
    row_idx, col_idx = input_adj.nonzero()  # .t().contiguous()
...
def aug_random_edge(input_adj, drop_edge=0.2):
    ...
    edge_num = int(len(row_idx))      # 9228 / 2
    add_drop_num = int(edge_num * percent)
    aug_adj = copy.deepcopy(input_adj.tolist())#copy.deepcopy(input_adj.todense().tolist())
    # aug_adj = copy.deepcopy(input_adj.todense().tolist())
    ...
    aug_adj = np.matrix(aug_adj)
    aug_adj = sp.csr_matrix(aug_adj)
    edge_index = from_scipy_sparse_matrix(aug_adj)[0]
    return edge_index
