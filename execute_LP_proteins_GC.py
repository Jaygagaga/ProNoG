...
parser.add_argument('--filename', type=str, default='no_prompt_with_metanet_origin_feature_LP_graph',
...
if args.dataset in ['BZR','COX2']:
    ...
    a = torch_geometric.utils.to_scipy_sparse_matrix(torch.tensor(edge_index).T)
    adj = sp.csr_matrix(a)
...
if args.dataset in ['ENZYMES', 'PROTEINS']:
    ...
    if not os.path.exists(f"data/{args.dataset}/data.pkl"):
        ...
        for step, data in enumerate(loader):
            features, adj, labels = process.process_tu(data, 1)  # 1-{args.dataset}, 1-PROTEINS
...
if args.dataset in ['BZR', 'COX2']:
    ...
    model = PrePrompt(ft_size, hid_units, args.num_layers, 0.05, None,
                      model_name=args.model_name, reduction=args.reduction, hop_level=args.hop_level, gp=args.gp,
                      weight=args.weight,
                      concat_dense=args.concat_dense, g=None)
else:
    from GP.preprompt import PrePrompt
...
LP = False
if args.dataset in ['BZR', 'COX2']:
    test_embs, _= model.embed(test_features, sp_adj_test, msk=None,LP=LP,edge_index=edge_index,)
else:
...
for shotnum in args.shotnum:
    ...
    for i in range(100):
        ...
        if args.dataset in ['BZR', 'COX2']:
            embeds, _ = model.embed(feature_train, sp_adj_train, msk=None, LP=LP, edge_index=edge_index, )
        else:
        ...
        with torch.no_grad():
            ...
            logits = log.forward_homo_graph(train=0, neighbors=testneighbors, neighbors_2hop=testneighbors_2hop,
