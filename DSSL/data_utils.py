...
# def dataset_check(data, args, device, transform=None):
#     # check the label dim
#     if len(data.y.shape) == 1:
#         data.y = data.y.unsqueeze(1)
#     data.y.label = data.y.to(device)
#     # node num, class num, feature dim
#     n = data.num_nodes
#     c = max(data.y.max().item() + 1, data.y.shape[1])
#     d = data.x.shape[1]
#     # split
#     if args.split == 'rand':
#         split_idx_lst = []
...
#                 data.y, train_prop=args.train_prop, valid_prop=args.valid_prop, ignore_negative=False)
#             split_idx = {'train': train_idx,
#                             'valid': valid_idx,
#                             'test': test_idx}
#             split_idx_lst.append(split_idx)
#     elif args.split == 'balanced':
#         split_idx_lst = []
#         for i in range(args.runs):
#             train_idx, valid_idx, test_idx = rand_balance_idx(
#                 data.y, c, train_prop=args.train_prop, valid_prop=args.valid_prop, ignore_negative=False)
#             split_idx = {'train': train_idx,
#                             'valid': valid_idx,
#                             'test': test_idx}
#             split_idx_lst.append(split_idx)
#     else:
#         split_idx_lst = load_fixed_splits(args.dataset, args.sub_dataset, data, args.runs)
#     # symmetrize check
...
#         data = data.to(device)
#     except:
#         print('Error:data not on Cuda')
#     print(f"num nodes {n} | num classes {c} | num node feats {d}")
#     return data, n, c, d, split_idx_lst
...
# def load_fixed_splits(dataset, sub_dataset, data, runs):
...
#         name += f'-{sub_dataset}'
#
#     # split: torch_geometric > file > None
...
#             for i in range(runs):
#                 train_idx, valid_idx, test_idx = data.train_mask, data.val_mask, data.test_mask
#                 split_idx = {'train': train_idx,
#                                 'valid': valid_idx,
#                                 'test': test_idx}
#                 splits_lst.append(split_idx)
#         elif dataset in ['chameleon', 'cornell', 'film', 'squirrel', 'texas', 'wisconsin']:
#             for i in range(runs):
#                 train_idx, valid_idx, test_idx = data.train_mask[:,i], data.val_mask[:,i], data.test_mask[:,i]
#                 split_idx = {'train': train_idx,
#                                 'valid': valid_idx,
#                                 'test': test_idx}
#                 splits_lst.append(split_idx)
